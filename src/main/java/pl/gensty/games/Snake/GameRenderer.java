...
import pl.gensty.games.Snake.apple.displayer.AppleDisplayer;
import pl.gensty.utils.Displayer;
import pl.gensty.games.Snake.apple.Apple;
import pl.gensty.games.Snake.snake.Snake;
import pl.gensty.games.Snake.snake.displayer.SnakeDisplayer;
...
public class GameRenderer {
    ...
    private void drawBoard(Graphics g) {
        ...
        for (int i = 0; i < SnakeSettings.SCREEN_HEIGHT / UNIT_SIZE; i++) {
            g.drawLine(i * UNIT_SIZE, 0, i * UNIT_SIZE, SnakeSettings.SCREEN_HEIGHT);
            g.drawLine(0, i * UNIT_SIZE, SnakeSettings.SCREEN_WIDTH, i * UNIT_SIZE);
        }
    }
    ...
    private void drawScore(Graphics g) {
        ...
        g.drawString(text, (SnakeSettings.SCREEN_WIDTH - metrics.stringWidth(text)) / 2, g.getFont().getSize());
    }
    ...
    private void drawGameOver(Graphics g) {
        ...
        g.drawString("Game Over", (SnakeSettings.SCREEN_WIDTH - metrics1.stringWidth("Game Over")) / 2, SnakeSettings.SCREEN_HEIGHT / 2 - 50);

        g.setColor(Color.WHITE);
        g.setFont(new Font("Gensty", Font.PLAIN, 25));
        FontMetrics metrics2 = g.getFontMetrics();
        String retryText = "PLAY AGAIN? (PRESS ENTER)";
        g.drawString(retryText, (SnakeSettings.SCREEN_WIDTH - metrics2.stringWidth(retryText)) / 2, SnakeSettings.SCREEN_HEIGHT / 2 + 20);
    }
